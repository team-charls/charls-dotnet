...
try
{
    ...
    try
    {
        ...
        {
            InterleaveMode = frameInfo.ComponentCount == 1 ? InterleaveMode.None : InterleaveMode.Sample
        };

        encoder.WriteStandardSpiffHeader(MapComponentCountToSpiffColorSpace(frameInfo.ComponentCount));
        encoder.Encode(pixels, bitmapData.Stride);

        Save(GetOutputPath(inputPath), encoder.EncodedData.Span);
    }
    finally
    {
