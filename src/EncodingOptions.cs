...
public enum EncodingOptions
{
    ...
    /// DICOM requires that data is always even. This can be done by adding manual a zero padding byte
    /// after the encoded data or with this option.
    /// </summary>
    EvenDestinationSize = 1,

    /// <summary>
    /// Add a comment (COM) segment with the content: "charls-dotnet [version-number]" to the encoded data.
